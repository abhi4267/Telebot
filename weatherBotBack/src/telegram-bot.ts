...
export class TelegramBotService {
  ...
  private registerCommands() {
    ...
    this.bot.onText(/\/start/, async(msg: any)=>{
      ...
      this.bot.sendMessage(chatId, `Hey there ${first_name}! Welcome to MyWeatherBot, you can subscribe using /subscribe command, and unsubscribe using /unsubscribe command}`)
      

    })
    this.bot.onText(/\/subscribe/, async (msg: any) => {
      console.log(msg);
      console.log(msg.chat.body);
      const chatId = msg.chat.id;
      ...
      if (existingUser) {
        this.bot.sendMessage(chatId, 'Already registered.');
      } else {
        const user = await this.userService.createUser(userId, username);
        if (user) {
          this.bot.sendMessage(chatId, 'Registration added');
    ...
    this.bot.onText(/\/unsubscribe/, async (msg) => {
      ...
      if (existingUser) {
        ...
        if (deletedUser) {
          ...
          this.bot.sendMessage(chatId, 'Unregistered.');
        } else {
          this.bot.sendMessage(chatId, 'Failed. Please try again.');
        }
      } else {
